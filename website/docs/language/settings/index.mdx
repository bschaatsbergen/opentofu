...
The special `opentofu` or `terraform` configuration block type is used to configure some
...
the `terraform` block stays as-is. In addition, we introduced an `opentofu` block for a more native Tofu experience.
:::


## OpenTofu Settings Block Syntax

OpenTofu settings can be defined using either the `opentofu` or `terraform` block. 
These blocks are functionally equivalent and can be used interchangeably; however, we recommend choosing one for consistency and clearer visual structure.
...
Each `opentofu` or `terraform` block can contain a number of settings related to OpenTofu's
behavior. Within a `opentofu` or `terraform` block, only constant values can be used;
...
The various options supported within a `opentofu` or `terraform` block are described in the
following sections.
...
The `opentofu` or `terraform` block can have a nested `provider_meta` block for each
